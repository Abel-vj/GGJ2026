...
Shader "Toony Colors Pro 2/User/ToonyTerrain"
{
	Properties
	{
		[TCP2HeaderHelp(Base)]
	...
	SubShader
	{
		...
		Pass
		{
			...
			) : SV_Target
			{
				...
				float4 __layer0NormalMap = ( TCP2_TEX2D_SAMPLE(_Normal0, _Normal0, input.pack2.xy * _Splat0_ST.xy + _Splat0_ST.zw).rgba );
				float4 __layer1NormalMap = ( TCP2_TEX2D_SAMPLE(_Normal1, _Normal0, input.pack2.xy * _Splat1_ST.xy + _Splat1_ST.zw).rgba );
				float4 __layer2NormalMap = ( TCP2_TEX2D_SAMPLE(_Normal2, _Normal0, input.pack2.xy * _Splat2_ST.xy + _Splat2_ST.zw).rgba );
				float4 __layer3NormalMap = ( TCP2_TEX2D_SAMPLE(_Normal3, _Normal0, input.pack2.xy * _Splat3_ST.xy + _Splat3_ST.zw).rgba );
				float4 __layer0Albedo = ( TCP2_TEX2D_SAMPLE(_Splat0, _Splat0, input.pack2.xy * _Splat0_ST.xy + _Splat0_ST.zw).rgba );
				float4 __layer1Albedo = ( TCP2_TEX2D_SAMPLE(_Splat1, _Splat0, input.pack2.xy * _Splat1_ST.xy + _Splat1_ST.zw).rgba );
				float4 __layer2Albedo = ( TCP2_TEX2D_SAMPLE(_Splat2, _Splat0, input.pack2.xy * _Splat2_ST.xy + _Splat2_ST.zw).rgba );
				float4 __layer3Albedo = ( TCP2_TEX2D_SAMPLE(_Splat3, _Splat0, input.pack2.xy * _Splat3_ST.xy + _Splat3_ST.zw).rgba );
	...
	Dependency "AddPassShader"    = "Hidden/Toony Colors Pro 2/User/ToonyTerrain-AddPass"
	Dependency "BaseMapShader"    = "Hidden/Toony Colors Pro 2/User/ToonyTerrain-BasePass"
	Dependency "BaseMapGenShader" = "Hidden/Toony Colors Pro 2/User/ToonyTerrain-BaseGen"

	FallBack "Hidden/InternalErrorShader"
	CustomEditor "ToonyColorsPro.ShaderGenerator.MaterialInspector_SG2"
}

/* TCP_DATA u config(ver:"2.9.21";unity:"6000.3.4f1";tmplt:"SG2_Template_URP";features:list["UNITY_5_4","UNITY_5_5","UNITY_5_6","UNITY_2017_1","UNITY_2018_1","UNITY_2018_2","UNITY_2018_3","UNITY_2019_1","UNITY_2019_2","UNITY_2019_3","UNITY_2019_4","UNITY_2020_1","UNITY_2021_1","UNITY_2021_2","UNITY_2022_2","UNITY_6000_2","UNITY_6000_1","UNITY_6000_0","ENABLE_DEPTH_NORMALS_PASS","ENABLE_FORWARD_PLUS","TERRAIN_SHADER","RAMP_BANDS","BUMP","SKETCH_PROGRESSIVE","SKETCH_SHADER_FEATURE","OUTLINE","OUTLINE_URP_FEATURE","TERRAIN_HEIGHT_BLENDING","SKETCH_PROGRESSIVE_SMOOTH","TEMPLATE_LWRP"];flags:list[];flags_extra:dict[];keywords:dict[RENDER_TYPE="Opaque",RampTextureDrawer="[TCP2Gradient]",RampTextureLabel="Ramp Texture",SHADER_TARGET="3.0",BASEGEN_ALBEDO_DOWNSCALE="1",BASEGEN_MASKTEX_DOWNSCALE="1/2",BASEGEN_METALLIC_DOWNSCALE="1/4",BASEGEN_SPECULAR_DOWNSCALE="1/4",BASEGEN_DIFFUSEREMAPMIN_DOWNSCALE="1/4",BASEGEN_MASKMAPREMAPMIN_DOWNSCALE="1/4"];shaderProperties:list[,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,sp(name:"Progressive Sketch Texture";imps:list[imp_mp_texture(uto:True;tov:"";tov_lbl:"";gto:False;sbt:False;scr:False;scv:"";scv_lbl:"";gsc:False;roff:False;goff:False;sin_anm:False;sin_anmv:"";sin_anmv_lbl:"";gsin:False;notile:False;triplanar_local:False;def:"black";locked_uv:False;uv:6;cc:4;chan:"RGBA";mip:-1;mipprop:False;ssuv_vert:False;ssuv_obj:False;uv_type:Triplanar;uv_chan:"XZ";tpln_scale:1;uv_shaderproperty:__NULL__;uv_cmp:__NULL__;sep_sampler:__NULL__;prop:"_ProgressiveSketchTexture";md:"";gbv:False;custom:False;refs:"";pnlock:False;guid:"1e01f518-562c-40b8-b02e-559164e198d1";op:Multiply;lbl:"Progressive Texture";gpu_inst:False;dots_inst:False;locked:False;impl_index:0)];layers:list[];unlocked:list[];layer_blend:dict[];custom_blend:dict[];clones:dict[];isClone:False)];customTextures:list[];codeInjection:codeInjection(injectedFiles:list[];mark:False);matLayers:list[]) */
/* TCP_HASH acf79aa578023ac873e02cd74fa0f553 */
